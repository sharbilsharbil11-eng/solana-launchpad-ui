...
**السياق:** بيئة العمل التي كتبت فيها هذا الكود محجوب عنها أي وصول لشبكة
سولانا (devnet/mainnet) بالكامل، وحتى تثبيت أدوات Solana/Anchor نفسها —
لذلك ما قدرت أبني (`anchor build`) أو أنشر (`anchor deploy`) البرنامج
بنفسي. كل الملفات جاهزة 100% وتم فحصها بعناية (راجع `idl/validate-idl.js`
و`idl/validate-manual-encoder.js`)، بس **النشر الفعلي يحتاج تنفَّذ من جهاز
عنده اتصال إنترنت حقيقي** (جهازك، أو أي بيئة CI عندها شبكة).
